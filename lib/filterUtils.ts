...
export function applyFilter(
  ...
  const inCreatedByIds  = new Set((include.createdBy ?? []).map(m => m.id))
  ...
  const exCreatedByIds  = new Set((exclude.createdBy ?? []).map(m => m.id))
  ...
  let result = issues.filter(issue => {
    ...
    if (inCreatedByIds.size > 0 && !inCreatedByIds.has(issue.created_by ?? ''))        return false
    ...
    if (exCreatedByIds.size > 0 && exCreatedByIds.has(issue.created_by ?? ''))         return false
